(() => {
  ...
  function accessToken() {
    try { return window.localStorage.getItem('cogneeSpaceAccessToken') || '' } catch { return '' }
  }

  function setAccessToken(token) {
    try {
      if (token) window.localStorage.setItem('cogneeSpaceAccessToken', token)
    } catch {}
  }
  ...
  function localPlan(fields) {
    ...
    const endpoints = []
    if (/remember|store|persist|durable|long[- ]term|preference|history|context/.test(goal)) endpoints.push('remember')
    if (/recall|retrieve|search|answer|query|semantic|graph/.test(goal)) endpoints.push('recall')
    if (/knowledge graph|graph rag|entity|relationship|ontology|vector|embedding|rag/.test(goal)) endpoints.push('graph_rag')
    if (/mcp|model context protocol|client|claude|cursor|stdio|sse/.test(goal) || deployment === 'mcp') endpoints.push('mcp')
    if (/tenant|permission|privacy|access|security|regulated|customer/.test(goal)) endpoints.push('access')
    if (/feedback|learn|improve|rating|thumb|self[- ]improving/.test(goal)) endpoints.push('feedback')
    if (/api|server|docker|postgres|neo4j|lancedb|self|backup|migration/.test(goal) || deployment === 'self_hosted' || scale === 'large') endpoints.push('self_host')
    if (!endpoints.length) endpoints.push('remember', 'recall', 'graph_rag')
    const unique = [...new Set(endpoints)]
    return {
      ok: true,
      product: data.brand || 'Cognee Space',
      status: 'planner_ready_offline',
      recommendedEndpoints: unique.map((name) => ({ name })),
      outputMode: output === 'json' ? 'structured JSON plan with memory metadata' : output,
      deploymentPath: deployment === 'self_hosted' ? 'self-host Cognee checklist first' : deployment === 'mcp' ? 'MCP server checklist first' : 'SDK or API workflow checklist first',
      nextSteps: [
      ...
      compliance: [
        ...
        deployment === 'self_hosted'
          ? 'Review the Apache-2.0 license, LLM provider terms, data retention, and backend access controls before self-hosting.'
          : 'Use official Cognee docs as source evidence, then keep planning support checkout on cognee.space.',
      ],
    }
  }
