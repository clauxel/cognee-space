...
const scriptVersion = '/app.js?v=20260625-cognee-space-d1'
const required = [
  'index.html',
  'ai-memory/index.html',
  'knowledge-graph-rag/index.html',
  'agent-memory/index.html',
  'mcp-memory-server/index.html',
  'self-host-cognee/index.html',
  'pricing/index.html',
  'checkout/index.html',
  'success/index.html',
  'cancel/index.html',
  ...
  'styles.css',
  'app.js',
  'assets/cognee-memory-graph.png',
  'favicon.svg',
  'site.webmanifest',
  'robots.txt',
  'sitemap.xml',
...
for (const file of textFiles) {
  ...
  if (/Firecrawl|firecrawl|web-data-workflow|scrape|crawler|AGPL-3\.0|Stirling|stirling|PDF workflow|pdf-workflow|merge PDF|redaction/i.test(text)) {
...
for (const needle of ['page_view', 'billing_toggle', 'setBillingMode', 'data-checkout-link-plan', 'checkoutBilling', 'cogneeSpaceAccessToken']) {
...
for (const residue of ['<td>Official docs</td>', '<td>Official site</td>', '<td>GitHub repo</td>']) {
  if (docs.includes(residue)) throw new Error('Docs page matrix CTA must stay in the Cognee Space funnel: ' + residue)
}
if (!docs.includes('official site www.cognee.ai') || !docs.includes('github.com/topoteretes/cognee')) {
  throw new Error('Docs page missing plain-text official source notes')
}

for (const page of ['ai-memory', 'knowledge-graph-rag', 'agent-memory', 'mcp-memory-server', 'self-host-cognee', 'pricing', 'checkout', 'docs']) {
  ...
  if (!text.includes('Cognee Space')) throw new Error('Page missing product entity: ' + page)
}

const sitemap = await readFile(new URL('sitemap.xml', root), 'utf8')
const urlCount = (sitemap.match(/<url>/g) || []).length
if (urlCount < 14) throw new Error('Sitemap has too few URLs: ' + urlCount)
for (const route of ['ai-memory', 'knowledge-graph-rag', 'agent-memory', 'mcp-memory-server', 'self-host-cognee', 'pricing', 'checkout', 'docs']) {
...
response = await handleRequest(new Request(local + '/api/planner', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({ goal: 'Remember customer support history, recall similar cases, use knowledge graph RAG, MCP access, tenant privacy, feedback learning, and self host with Docker', scale: 'large', output: 'json', deployment: 'self_hosted' }),
}), { SITE_ASSETS: assetBinding })
...
}), { SITE_ASSETS: assetBinding, ACCESS_TEST_MODE: 'true', COGNEE_SPACE_ACCESS_SECRET: 'local_validation_secret' })
...
response = await handleRequest(new Request(local + '/api/planner', {
  ...
  body: JSON.stringify({ goal: 'Remember customer support history, recall similar cases, use knowledge graph RAG, MCP access, tenant privacy, feedback learning, and self host with Docker', scale: 'large', output: 'json', deployment: 'self_hosted' }),
}), { SITE_ASSETS: assetBinding, COGNEE_SPACE_ACCESS_SECRET: 'local_validation_secret' })
if (response.status !== 200) throw new Error('/api/planner did not return 200 after paid access')
const plan = await response.json()
if (!plan.recommendedEndpoints?.some((endpoint) => endpoint.name === 'remember') || !plan.recommendedEndpoints?.some((endpoint) => endpoint.name === 'recall') || !plan.recommendedEndpoints?.some((endpoint) => endpoint.name === 'graph_rag') || !plan.recommendedEndpoints?.some((endpoint) => endpoint.name === 'self_host')) {
  throw new Error('planner response is missing expected AI memory recommendations')
}

response = await handleRequest(new Request(local + '/.well-known/cognee-space.json'), { SITE_ASSETS: assetBinding })
if (response.status !== 200) throw new Error('facts JSON did not return 200')
const facts = await response.json()
if (facts.relationship !== 'independent_unofficial_reference' || !facts.upstream.license.includes('Apache-2.0')) {
