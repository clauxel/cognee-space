...
const site = 'cognee-space'
const domain = 'cognee.space'
const origin = 'https://cognee.space'
const today = new Date().toISOString().slice(0, 10)

const plans = [
  { plan: 'starter', name: 'Starter', monthlyCents: 900, summary: 'One AI memory rollout brief for a solo builder or internal tool owner.' },
  { plan: 'pro', name: 'Pro', monthlyCents: 2900, summary: 'Reviewable annual Cognee deployment plan, source notes, and evaluation checklist.' },
  { plan: 'enterprise', name: 'Enterprise', monthlyCents: 5900, summary: 'Governance notes, backend selection, and stakeholder-ready memory-system evidence.' },
]
...
async function ensureProduct(plan, period, existingProducts) {
  ...
  const productName = `Cognee Space ${plan.name} ${period.billing[0].toUpperCase()}${period.billing.slice(1)}`
...
async function ensureCheckoutLink(product, price, plan, period, existingLinks) {
  ...
  return polar('/v1/checkout-links/', {
    ...
    body: JSON.stringify({
      ...
      label: `Cognee Space ${plan.name} ${period.billing}`,
...
console.log(`Created or reused ${products.length} Cognee Space Polar checkout links.`)
