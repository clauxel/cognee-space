const CONFIG = {
  ...
  brand: 'Cognee Space',
  domain: 'cognee.space',
  canonicalOrigin: 'https://cognee.space',
  support: 'support@aigeamy.com',
  officialSite: 'https://www.cognee.ai',
  officialDocs: 'https://docs.cognee.ai/',
  officialRepo: 'https://github.com/topoteretes/cognee',
  license: 'Apache-2.0',
  summary: 'Independent, unofficial Cognee deployment planner for teams standardizing persistent AI memory, knowledge graph RAG, MCP memory servers, and self-hosted agent context.',
  endpoints: {
    remember: 'Define what the agent stores permanently, what remains session-scoped, and how datasets are named.',
    recall: 'Choose semantic, graph, temporal, scoped, or session-first recall behavior for future prompts.',
    graph_rag: 'Map documents, entities, relationships, embeddings, and graph reasoning into a reviewable RAG plan.',
    mcp: 'Plan Cognee MCP server transports, API mode, Docker mode, client setup, logging, and deletion behavior.',
    self_host: 'Review LLM keys, Docker profiles, API server, frontend, graph/vector backends, migrations, and backups.',
    access: 'Set tenant, user, dataset, and memory-review boundaries before storing private or regulated context.',
    feedback: 'Capture thumbs-up/down signals, importance weights, and memory promotion rules for self-improving agents.',
    observability: 'Track memory writes, recalls, paid-gate hits, checkout starts, AI referrals, and error cases in D1.',
  },
  defaultPlanId: 'pro',
  ...
  plans: {
    starter: {
      ...
      summary: 'One AI memory rollout brief for a solo builder or internal tool owner.',
    },
    pro: {
      id: 'pro',
      name: 'Pro',
      ...
      defaultBilling: 'annual',
      allowedBilling: ['monthly', 'annual'],
      summary: 'A reviewable annual Cognee deployment plan, source notes, and evaluation checklist for a team.',
    },
    enterprise: {
      id: 'enterprise',
      name: 'Enterprise',
      ...
      defaultBilling: 'annual',
      allowedBilling: ['monthly', 'annual'],
      summary: 'Governance notes, backend selection, and stakeholder-ready memory-system evidence for larger teams.',
    },
  },
}
...
const POLAR_GENERIC_PRODUCT_ID_KEYS = ['POLAR_PRODUCT_ID', 'POLAR_DEFAULT_PRODUCT_ID', 'POLAR_COGNEE_SPACE_PRODUCT_ID']
...
const ACCESS_SECRET_KEYS = ['COGNEE_SPACE_ACCESS_SECRET', 'ACCESS_SIGNING_SECRET']
...
async function handleAccess(request, env) {
  ...
  if (!signingSecret || !accessToken) {
    return jsonResponse({
      ...
      missing: ['POLAR_ACCESS_TOKEN', 'COGNEE_SPACE_ACCESS_SECRET'],
    }, 503, request)
  }
...
async function handleCheckout(request, env, requestUrl) {
  ...
  const orderId = `cognee_space_${planId}_${billing}_${Date.now()}_${Math.random().toString(16).slice(2)}`
  ...
  if (!accessToken || !productId) {
    return jsonResponse({
      ...
      provider: 'polar',
      error: 'Polar checkout is not configured yet for this Cognee Space plan.',
...
async function handleAnalytics(request, env, requestUrl) {
  ...
  if (!sinks.length) {
    return jsonResponse({
      ok: true,
      stored: false,
      provider: 'cognee-space-analytics',
...
function planFor(body = {}) {
  const goal = String(body.goal || 'agent needs persistent memory').toLowerCase()
  const scale = String(body.scale || 'documents').toLowerCase()
  const output = String(body.output || 'checklist').toLowerCase()
  const deployment = String(body.deployment || 'sdk').toLowerCase()
  const needsRemember = /remember|store|persist|durable|long[- ]term|preference|history|context/.test(goal)
  const needsRecall = /recall|retrieve|search|answer|query|semantic|graph/.test(goal)
  const needsGraphRag = /knowledge graph|graph rag|entity|relationship|ontology|vector|embedding|rag/.test(goal)
  const needsMcp = /mcp|model context protocol|client|claude|cursor|stdio|sse/.test(goal) || deployment === 'mcp'
  const needsAccess = /tenant|permission|privacy|access|security|regulated|customer/.test(goal)
  const needsFeedback = /feedback|learn|improve|rating|thumb|self[- ]improving/.test(goal)
  const needsSelfHost = /api|server|docker|postgres|neo4j|lancedb|self|backup|migration/.test(goal) || deployment === 'self_hosted' || scale === 'large'

  const endpoints = []
  if (needsRemember) endpoints.push('remember')
  if (needsRecall) endpoints.push('recall')
  if (needsGraphRag) endpoints.push('graph_rag')
  if (needsMcp) endpoints.push('mcp')
  if (needsAccess) endpoints.push('access')
  if (needsFeedback) endpoints.push('feedback')
  if (needsSelfHost) endpoints.push('self_host')
  if (!endpoints.length) endpoints.push('remember', 'recall', 'graph_rag')

  const uniqueEndpoints = [...new Set(endpoints)]
  const compliance = [
    'Classify memory sensitivity before ingestion, graph extraction, retrieval, or cross-agent reuse.',
    'Keep source names, timestamps, dataset names, recall settings, and evaluation examples with the workflow evidence.',
  ]
  if (needsSelfHost) {
    compliance.push('Self-hosting requires LLM key handling, backend selection, migration checks, backups, access control, and observability before production.')
    compliance.push('Review the Apache-2.0 license, LLM provider terms, data retention, and backend access controls before self-hosting.')
  } else {
    compliance.push('Use the official Cognee docs and repository as source evidence, while keeping hosted planning support and checkout on cognee.space.')
  }
  ...
  return {
    ...
    outputMode: output.includes('json') ? 'structured JSON plan with memory metadata' : output,
    deploymentPath: needsSelfHost ? 'self-host Cognee checklist first' : needsMcp ? 'MCP server checklist first' : 'SDK or API workflow checklist first',
    nextSteps: [
      'Choose dataset names, node sets, and recall modes before storing production memory.',
      'Run a small evaluation set through remember, recall, forget, and improve before opening team access.',
      'Add access controls, deletion expectations, feedback capture, storage backups, and observability before production automation.',
    ],
    compliance,
    sourceOfTruth: {
...
export async function handleRequest(request, env) {
  ...
  if (url.pathname === '/.well-known/cognee-space.json') {
    return jsonResponse({
      name: CONFIG.brand,
